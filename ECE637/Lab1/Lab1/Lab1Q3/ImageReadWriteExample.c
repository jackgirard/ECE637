...
void applyFilter(struct TIFF_img* output_img, struct TIFF_img* input_img, double** usedFilter, int PSF_dim);
...
int main (int argc, char **argv)
{
    ...
    // populate filter array according to h(m,n)
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            filter[i][j] = 1.0 / 81.0;
        }
    ...
    applyFilter(&color_img, &input_img, filter, PSF_dim);
...
void applyFilter(struct TIFF_img* output_img, struct TIFF_img* input_img, double** usedFilter, int PSF_dim) {
    ...
    for (int i = 0; i < img_height; i++) {
        for (int j = 0; j < img_width; j++) {
            ...
            greenPlane = 0.0;
            bluePlane = 0.0;
            // for each pixel in the PSF (9*9 in this case)
            for (m = -N; m <= N; m++) {
                for (n = -N; n <= N; n++) {
